...
def check_store(cfg, store, now, live):
    ...
    # it. one shop started doing this on 2026-09-10.
    if want.get('speakers'):
...
def point_at_the_live_group(store):
    ...
    Written 2026-09-10, the morning two shops both went silent. A group id
...
def main():
    ...
    # thing nobody could answer when one shop's died on 2026-09-10.
    ages = []
    for s in enabled:
